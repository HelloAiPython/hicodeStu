...
class LeaderboardApiTests(BaseApiFixture):
    ...
    def test_score_audit_logs_created_by_bulk_actions(self):
        enrollment = self.students[2][1]
        self.client.post(
            "/api/classroom-scores/bulk_create/",
            {"records": [{"enrollment": enrollment.id, "attentive": 88}]},
            format="json",
        )
        self.client.post(
            "/api/homework-scores/bulk_create/",
            {"records": [{"enrollment": enrollment.id, "completion": 87}]},
            format="json",
        )
        actions = list(ScoreAuditLog.objects.values_list("action", flat=True))
        self.assertIn("classroom_score_bulk_create", actions)
        self.assertIn("homework_score_bulk_create", actions)

    def test_score_audit_log_list(self):
...
class ReportAndProgressApiTests(BaseApiFixture):
    ...
    def test_student_detail_dashboard(self):
        student_profile = self.students[1][0]
        response = self.client.get(f"/api/students/{student_profile.id}/detail_dashboard/?threshold=80")
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["student_number"], "S002")
...
class StudentPermissionTests(BaseApiFixture):
    def setUp(self):
        super().setUp()
        self.client.force_authenticate(user=self.students[0][0].user)

    def test_student_list_only_contains_self(self):
        response = self.client.get("/api/students/")
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(len(data), 1)
        self.assertEqual(data[0]["student_number"], "S001")

    def test_student_can_view_own_progress(self):
        student_profile = self.students[0][0]
        response = self.client.get(f"/api/students/{student_profile.id}/progress/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["student_number"], "S001")

    def test_student_me_profile(self):
        response = self.client.get("/api/students/me/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["student_number"], "S001")

    def test_student_me_progress(self):
        response = self.client.get("/api/students/me/progress/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["student_number"], "S001")

    def test_student_me_alerts(self):
        response = self.client.get("/api/students/me/alerts/?threshold=95")
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["student_number"], "S001")
        self.assertEqual(data["risk_count"], 1)
    ...
    def test_student_cannot_view_other_student_progress(self):
        other_student = self.students[1][0]
        response = self.client.get(f"/api/students/{other_student.id}/progress/")
        self.assertEqual(response.status_code, 404)

    def test_student_cannot_access_teacher_dashboard(self):
        response = self.client.get("/api/students/alerts_board/")
        self.assertEqual(response.status_code, 403)

    def test_teacher_cannot_access_student_me_endpoint(self):
        self.client.force_authenticate(user=self.teacher)
        response = self.client.get("/api/students/me/")
        self.assertEqual(response.status_code, 403)
